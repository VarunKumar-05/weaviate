...
func (s *Shard) initVectorIndex(ctx context.Context,
	...
	if lazyLoadSegments != s.lazySegmentLoadingEnabled {
		options := []lsmkv.BucketOption{lsmkv.WithLazySegmentLoading(lazyLoadSegments)}
		makeBucketOptions = func(strategy string, customOptions ...lsmkv.BucketOption) []lsmkv.BucketOption {
			return s.makeDefaultBucketOptions(strategy, append(options, customOptions...)...)
		}
	}
